...
exports.getAccurateClimateData = getAccurateClimateData;
 ...
 */
async function getAccurateClimateData(latitude, longitude) {
    try {
        ...
        const snowIndex = calculateSnowIndexFromClimate(climateType, weatherData.avgTemp, weatherData.totalSnow, weatherData.snowDays);
        const stormIndex = calculateStormIndexFromData(weatherData.avgWindSpeed, weatherData.rainyDays, latitude, climateType);
        const result = {
 ...
 */
function determineClimateType(latitude, avgTemp) {
    ...
    // Subtropical: warm but with some seasonal variation
        ...
        if (avgTemp > 15)
            return "Subtropical"; // Miami, Cairo, Sydney
        else
            return "Temperate";
    }
 ...
 */
async function getOpenMeteoHistoricalData(latitude, longitude) {
    try {
 ...
 */
function estimateRainyDays(latitude, avgTemp) {
    const absLat = Math.abs(latitude);
    if (absLat < 23.5) {
        // Tropical: 100-150 rainy days (monsoon + scattered)
        if (avgTemp > 25)
            return 130; // Hot tropical
        else
            return 110;
    }
    else if (absLat < 35) {
 ...
 * Used when API data unavailable
 */
function estimateSnowDays(latitude, avgTemp) {
    const absLat = Math.abs(latitude);
    if (absLat < 23.5) {
 ...
 */
function calculateSnowIndexFromClimate(climateType, avgTemp, totalSnow, snowDays) {
 ...
 */
function calculateStormIndexFromData(avgWindSpeed, rainyDays, latitude, climateType) {
