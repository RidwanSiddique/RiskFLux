 ...
 * 10+km = 0-20 (low risk)
 */
function normalizeFloodDistance(km: number): number {
 ...
 * 500m+ = <30 (low risk)
 */
function normalizeFloodElevation(elevation: number): number {
    if (elevation <= 10) return 100; // Sea level/coastal areas
    if (elevation <= 50) return 90;
    if (elevation <= 100) return 80;
    if (elevation <= 200) return 60;
    if (elevation <= 300) return 40;
    if (elevation <= 500) return 25;
    if (elevation <= 1000) return 10;
